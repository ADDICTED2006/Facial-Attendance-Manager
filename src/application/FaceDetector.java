						...
						if (faces != null) {
							...
							for (int i = 0; i < total; i++) {
								...
								if (isRecFace) {
									...
									this.recogniseCode = faceRecognizer.recognize(temp)[0];
									this.recogniseConf = faceRecognizer.recognize(temp)[1];
									...
									if(recogniseCode != -1)
									{
										...
										this.output = user;
										names = user.get(1) + " " + user.get(2) + this.recogniseConf;
										database.markAttendance(this.recogniseCode);
									}
